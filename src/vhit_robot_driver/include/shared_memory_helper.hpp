...
public:
   ...
   * @param[out] what Status or error details for logging.
   * @param force If true writes all supported cached variables with valid values.
  ...
  /**
   * @brief Return a copy of the cached variable metadata table.
   * @return Variables indexed by memory-map relative name.
   */
  std::unordered_map<std::string, SharedMemoryVariable> getVariables();

  /**
   * @brief Begin a nonblocking realtime shared-memory access window.
   * @param[out] firstBytePtr Pointer to the first byte of the memory image on DL_OK.
   * @return DL_OK on success, DL_RT_WOULD_BLOCK when nonblocking access would block,
   * or another Data Layer realtime error.
   */
  const comm::datalayer::DlResult beginAccessRt(uint8_t * & firstBytePtr);

  /**
   * @brief End the current realtime shared-memory access window.
   * @return DL_OK on success or a Data Layer realtime error.
   */
  const comm::datalayer::DlResult endAccessRt();

  /**
   * @brief Read one 32-bit signed value from an active realtime memory window.
   * @param[in] byteIndex Byte offset from the beginning of the memory area.
   * @param[in] firstBytePtr Pointer returned by beginAccessRt().
   * @param[out] val Raw DINT value copied from memory.
   */
  void readVariableRt(const uint32_t byteIndex, const uint8_t * firstBytePtr, int32_t & val);

  /**
   * @brief Write one 32-bit signed value into an active realtime memory window.
   * @param[in] byteIndex Byte offset from the beginning of the memory area.
   * @param[in,out] firstBytePtr Pointer returned by beginAccessRt().
   * @param[in] val Raw DINT value to copy into memory.
   */
